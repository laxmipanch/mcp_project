# contains mcp and claude client
import os
import json
import requests
import time
...
class ClaudeClient:
    def __init__(self, api_key: str = CLAUDE_API_KEY, model: str = "claude-3-opus-20240229"):
        ...
        self.tools = [{
            ...
            "description" : "retrives info from website based on user query",
            "input_schema" : {
                "type" : "object",
                "properties" : {
                    "query" : {
                ...
                for content_block in result.get("content", []):
                    if content_block.get("type") == "tool_use":
                        ...
                        conversation_history.append({"role" : "assistant", "content" :[
                            {
                                "type" : "text",
                                "text" : result.get("content", [{}]) [0].get("text", "") + "\n\n The tool call was successful. here is the tool response: "+ tool_response["results"][0]["description"]
                            }
                        ]})
        ...
        while retry_count<max_retries:
            try:
                ...
                if retry_count<max_retries:
                    ...
                    return{
                        "error" : f"mcp server not responding with error {e}"
                    }
